...
void* serverMenu(void* arg)
{
    ...
    if (cliMode == 1)
    {
        printf("client: Professor mode starts\n"); //debug
        ...
        while (1) //★
        {
            ...
            if (strcmp(check, "없음") != 0) //아이디가 리스트에 있으면과목인증 진행
            {
                printf("과목명 받기직전\n");
                ...
                while (1) //교수가 접근할 수 있는 과목리스트
                {
                    ...
                    printf("while들어왔습니다.\n");   //디버깅문항
                    printf("num: %d", num);           //디버깅 문항 과목수,인덱스
                    title = readFile(subj_list, num); //과목이름 한줄씩읽기

                    printf("해당 교수의 과목 리스트 파일 한줄을 읽었습니다.: %s\n\n", title); //디버깅문항
    ...
    else if (cliMode == 2)
    {
        ...
        char* check = CheckID(mode, idRcv);                               ///// msg: 학생아이디.txt or 없음
        printf("ID authentication succeeds\nCheckId result:%s\n", check); //debug

        char* ACResult = CheckClient(check);

        printf("ACResult returns \nACResult:%s\n\n", ACResult); //debug
        sndMsg(cliSockFd, ACResult);
        ...
        if (strcmp(check, "없음") != 0) //학번이 리스트에 있으면 진행 //strcmp로 수정
        {
            ...
            while (1)
            {
                ...
                if (title == NULL)
                {
                printf("사용자한테 \"끝\"이라고 보내겠어\n");
                    //write(cliSockFd, "끝", 100);
